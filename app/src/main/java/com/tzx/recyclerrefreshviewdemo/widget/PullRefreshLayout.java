package com.tzx.recyclerrefreshviewdemo.widget;
...
public class PullRefreshLayout extends ViewGroup {
    ...
    private View mTarget;   //需要刷新的view
    private ImageView mRefreshView;    //刷新动画
    private Interpolator mDecelerateInterpolator;   //动画播放插值器
    private int mTouchSlop;     //滑动的最小距离
    private int mSpinnerFinalOffset;
    private int mTotalDragDistance;     //滑动的最远距离
    private RefreshDrawable mRefreshDrawable;
    private int mCurrentOffsetTop;  //mTarget距离parent顶部的距离
    private boolean mRefreshing;    //是否执行刷新
    private int mActivePointerId;       //活跃的触摸点
    private boolean mIsBeingDragged;        //是否开始拖动
    private float mInitialMotionY;  //准备滑动时的触摸点的Y坐标
    private int mFrom;
    ...
    private int mInitialOffsetTop;      //准备滑动时刷新组件距离父view的top值
    private boolean mDispatchTargetTouchDown;   //是否往下分发触摸事件
    private float mDragPercent;     //拖动最长距离的百分比
    ...
    public PullRefreshLayout(Context context, AttributeSet attrs) {
        ...
        Log.d("tanzhenxing","Constructor mTouchSlop=" + mTouchSlop);
...
//        mRefreshView.measure(MeasureSpec.makeMeasureSpec(mRefreshViewWidth, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(mRefreshViewHeight, MeasureSpec.EXACTLY));
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                Log.d("tanzhenxing","onInterceptTouchEvent  ACTION_DOWN");
                if (!mRefreshing) {
            ...
            case MotionEvent.ACTION_MOVE:
                Log.d("tanzhenxing","onInterceptTouchEvent  ACTION_MOVE mCurrentOffsetTop=" + mCurrentOffsetTop + "  mRefreshing=" + mRefreshing);
            ...
            case MotionEvent.ACTION_CANCEL:
                Log.d("tanzhenxing","onInterceptTouchEvent  ACTION_CANCEL||ACTION_UP");
                mIsBeingDragged = false;
                mActivePointerId = INVALID_POINTER;
                break;
            case MotionEventCompat.ACTION_POINTER_UP:
                Log.d("tanzhenxing","onInterceptTouchEvent  ACTION_POINTER_UP");
    ...
    * */
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                Log.d("tanzhenxing","onTouchEvent  ACTION_MOVE");
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN:
                Log.d("tanzhenxing","onTouchEvent  ACTION_POINTER_DOWN");
                final int index = MotionEventCompat.getActionIndex(ev);
                int mActivePointerTmpId = MotionEventCompat.getPointerId(ev, index);
                mInitialMotionY += getMotionEventY(ev, mActivePointerTmpId) - getMotionEventY(ev, mActivePointerId);
                mActivePointerId = mActivePointerTmpId;
                Log.d("tanzhenxing", "ACTION_POINTER_DOWN  " + "index=" + index + "  mActivePointerId=" + mActivePointerId);
                break;
            case MotionEventCompat.ACTION_POINTER_UP:
                Log.d("tanzhenxing","onTouchEvent  ACTION_POINTER_UP");
                onSecondaryPointerUp(ev);
                break;
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL: {
                Log.d("tanzhenxing","onTouchEvent  ACTION_CANCEL||ACTION_UP");
                if (mActivePointerId == INVALID_POINTER) {
                    return false;
                }
                if (mRefreshing) {
    ...
    private void animateOffsetToStartPosition() {
        Log.d("tanzhenxing","animateOffsetToStartPosition");
    ...
    private void animateOffsetToCorrectPosition() {
        Log.d("tanzhenxing","animateOffsetToCorrectPosition");
        mFrom = mCurrentOffsetTop;
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            ...
            Log.d("tanzhenxing","mRefreshListener  mCurrentOffsetTop=" + mCurrentOffsetTop);
        }
    };
...
//            mRefreshDrawable.stop();
            mRefreshView.setVisibility(View.GONE);
            mCurrentOffsetTop = mTarget.getTop();
            Log.d("tanzhenxing","mToStartListener  mCurrentOffsetTop=" + mCurrentOffsetTop);
        }
    };
    ...
    * */
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        Log.d("tanzhenxing","ACTION_POINTER_UP  " + "pointerIndex=" + pointerIndex + "  pointerId=" + pointerId);
        if (pointerId == mActivePointerId) {
            //final int newPointerIndex = pointerIndex == 0 ? 1 : 0;
            //mActivePointerId = MotionEventCompat.getPointerId(ev, newPointerIndex);
            int mActivePointerTmpId = MotionEventCompat.getPointerId(ev, 0);
            mInitialMotionY += getMotionEventY(ev, mActivePointerTmpId) - getMotionEventY(ev, mActivePointerId);
            mActivePointerId = mActivePointerTmpId;
            Log.d("tanzhenxing","mActivePointerId=" + mActivePointerId);
        }
    }
...
//        mRefreshView.bringToFront();
        Log.d("tanzhenxing","setTargetOffsetTop  mCurrentOffsetTop=" + mCurrentOffsetTop + " offset=" + offset);
